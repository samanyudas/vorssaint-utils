...
- **Notch.** An optional black home for music, per-app volume mixing, brightness, clipboard history, captures, parked files, system details and everyday tools at the top of the screen, including Macs without a camera notch. Idle is empty by default; active music appears in a horizontal strip with artwork and title beside the camera. Collapsed indicators stay within the menu bar and adapt to its available space, giving app menus priority. Content comes first, with compact navigation at the bottom. Playback and volume stay visible together in the music panel, and the empty player takes less room. Choose idle indicators and automatic music activity independently. Hover for a small shortcut strip, or click to open a panel that stays open until dismissed. Choose a narrower compact layout, a spacious layout, or your own width and maximum height. Controls adapt to the visible shortcuts. Arrange modules and hide or reorder individual controls. Optional trackpad feedback accompanies opening and switching sections. Disable Notch to keep the separate panels. Route the app panel, quick panel, capture chooser and file shelf independently. Capture controls open without initially dimming the desktop. The base stays opaque black in both appearances, with optional Liquid Glass controls. It appears in screenshots and recordings by default, with an option to hide it.
